...
public class XacmlParser {
	...
	/**
	 * Root of JAXB document. Choosing the type of the root element, i.e. request, policy, policy set
	 * @param d JAXB document
...
//				st.append("\n }");
	...
	/**
	 * Parsing of XACML PolicySet element
	 * A PolicySet can contain a policy, a policy set or a referenceID
	 * @param polSet as JAXB element
	 * @param st
	 * @return
	 */
	...
	protected StringBuffer parsePolicy(PolicyType policy, StringBuffer st) {
		...
		if (policy.getPolicyId()!=""){
			st.append("Policy "+ parseID(policy.getPolicyId()) + " < ");
		}else{
			//add unique name to polSet
	...
	/**
	 * Match functions of AllOf items inside AnyOf items recursevly inside a Target element
	 * Only designator is supported
	 * @param c
	 * @return
	 */
	private String parseMatch(MatchType c) {
		...
		s.append(parseValue(c.getAttributeValue()) + ", "); //funticon matching value
		if (c.getAttributeSelector() != null){
			s.append("---Selector Not Supported---");
			l.debug("Selector Not Supported");
		}else {
			//designator
			s.append(parseDesignator(c.getAttributeDesignator()));
		}
		s.append(")");
		return s.toString();
	}
	...
	/**
	 * Not requested categoryId of the AttributeAssignementExpression (in XACML is Optionally)
	 * Parsing only attributeId and expression
	 * @param argumentAssignementExpression of AdviceExpression and ObligationExpression
	 * @return parsed string in FACPL-like syntax
	 */
	private String parseArgumentObls(AttributeAssignmentExpressionType arg) {
		StringBuffer s = new StringBuffer();
		//structured name
		//no attribute id for arguments, it is decided by an agreement with policy writer
	...
	/**
	 * Condition: unique expression that is parsed according to the ExpressionType rule
	 * @param con
	 * @return
	 */
	 ...
	 *-> VariableReference - Not Supported
	 *-> FunctionType - Not Supported
	 *-> ApplyType - Supported 
	 *-> AttributeSelector (use Xpath)-> Not Supported
	 *-> AttributeDesignator - Supported
	 *-> AttributeValue - Supported
	 * @param expression
	 * @return
	 */
	...
	/**
	 * Parse <Apply> function. We admit nested function, that are recursively parsed
	 * @param ob
	 * @return
	 */
	...
	/**
	 * Remove xacml namespace and return the identifier
	 * Available for AttributeID, Category, Type and Function
	 * @param id
	 * @return
	 */
	...
	/**
	 * Parsing Request. <b> Notably multiple-values attributes MUST be expressed as a sequence of AttributeValue within
	 * the same <Attribute> element.
	 * @param RequestType
	 * @param StringBuffer st
