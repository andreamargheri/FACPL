	...
	public PolicySet_obligation_1(){
		...
		private class Rule_RuleD extends Rule {
				...
				Rule_RuleD (){
					...
					//Target
					addTarget(new ExpressionBooleanTree(ExprBooleanConnector.AND,new ExpressionBooleanTree(new ExpressionFunction(new it.unifi.facpl.lib.function.comparison.Equal(), "Mark",new AttributeName("subject","id") 
					)),new ExpressionBooleanTree(new ExpressionFunction(new it.unifi.facpl.lib.function.comparison.In(), new AttributeName("action","id") 
					,new Set(
					"write",
					"checkout"
