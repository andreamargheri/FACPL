...
;################### STRING DECLARATIONs #######################
 (declare-datatypes () ((String s_doctor s_e-Prescription s_read s_Alice s_e-Pre-Write s_e-Pre-Read s_pharmacist s_write s_Data_request_by_unauthorised_subject  s_AdditionalStringValue )))
...
;################################ TOP-LEVEL POLICY AliceEPrescription CONSTRAINTs ###########################
;##### Policy Target
(define-fun cns_target_AliceEPrescription () (TValue Bool)
	(equalString const_Alice n_resource/patient-id)
)
;##### Policy Obligations
(define-fun cns_obl_permit_AliceEPrescription ()  Bool
	 (and true))
 
(define-fun cns_obl_deny_AliceEPrescription ()  Bool
	 (and (and
...
;##### Policy Combining Algorithm
(define-fun cns_AliceEPrescription_cmb_final_permit () Bool
	 (or cns_ePre_permit cns_ruleDeny_permit)
)

(define-fun cns_AliceEPrescription_cmb_final_deny () Bool
	 (or 
		 (and cns_ePre_deny cns_ruleDeny_deny)
		 (and cns_ePre_deny cns_ruleDeny_notApp)
		 (and cns_ePre_notApp cns_ruleDeny_deny)
	 )
)

(define-fun cns_AliceEPrescription_cmb_final_notApp () Bool
	 (and cns_ePre_notApp cns_ruleDeny_notApp)
)

(define-fun cns_AliceEPrescription_cmb_final_indet () Bool
	 (or 
		 (and cns_ePre_indet (not cns_ruleDeny_permit))
		 (and (not cns_ePre_permit) cns_ruleDeny_indet)
	 )
)
...
;PERMIT
(define-fun cns_AliceEPrescription_permit () Bool
	(and 
		(isTrue cns_target_AliceEPrescription)
		cns_AliceEPrescription_cmb_final_permit
		cns_obl_permit_AliceEPrescription
	)
)
;DENY
(define-fun cns_AliceEPrescription_deny () Bool
	(and 
		(isTrue cns_target_AliceEPrescription)
		cns_AliceEPrescription_cmb_final_deny
		cns_obl_deny_AliceEPrescription
	)
)
;NOT APP
(define-fun cns_AliceEPrescription_notApp () Bool
	(or
		(or (isFalse cns_target_AliceEPrescription) (miss cns_target_AliceEPrescription))
		(and (isTrue cns_target_AliceEPrescription) cns_AliceEPrescription_cmb_final_notApp)
	)
)
;INDET
(define-fun cns_AliceEPrescription_indet () Bool
	(or 
		(not
			(or  
				(isBool cns_target_AliceEPrescription)
				(miss cns_target_AliceEPrescription)
			)
		)
		(and (isTrue cns_target_AliceEPrescription) cns_AliceEPrescription_cmb_final_indet)
		(and 
			(isTrue cns_target_AliceEPrescription)
			cns_AliceEPrescription_cmb_final_permit
			(not cns_obl_permit_AliceEPrescription)
		)
		(and 
			(isTrue cns_target_AliceEPrescription)
			cns_AliceEPrescription_cmb_final_deny
			(not cns_obl_deny_AliceEPrescription)
		)
	)
)
;################### END TOP-LEVEL POLICY AliceEPrescription CONSTRAINTs #########################
