...
;###################### STRUCTURAL PROPERTY #####################
(echo "--> Check Policy2COVERPolicy1_c... (holds if the following two checks are unsat)")
(echo "...(1/2) Policy2_PERMIT  COVER Policy1_c_PERMIT...(holds if it is unsat)")
 (push)
(assert 
	 (and (not cns_Policy2_permit) cns_Policy1_c_permit)
)
...
(pop)
(push)
(echo "...(2/2) Policy2_DENY  COVER Policy1_c_DENY...(holds if it is unsat)")
 (assert 
	 (and (not cns_Policy2_deny) cns_Policy1_c_deny)
)
