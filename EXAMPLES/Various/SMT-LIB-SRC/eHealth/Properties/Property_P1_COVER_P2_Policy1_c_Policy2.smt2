...
;###################### STRUCTURAL PROPERTY #####################
(echo "--> Check Policy1_cCOVERPolicy2... (holds if the following two checks are unsat)")
(echo "...(1/2) Policy1_c_PERMIT  COVER Policy2_PERMIT...(holds if it is unsat)")
 (push)
(assert 
	 (and (not cns_Policy1_c_permit) cns_Policy2_permit)
)
...
(pop)
(push)
(echo "...(2/2) Policy1_c_DENY  COVER Policy2_DENY...(holds if it is unsat)")
 (assert 
	 (and (not cns_Policy1_c_deny) cns_Policy2_deny)
)
