...
;################### START CONSTRAINT POLICY SET ePre #######################
...
;################### END CONSTRAINT POLICY SET ePre #########################
...
;################################ TOP-LEVEL POLICY AliceConsent CONSTRAINTs ###########################
;##### Policy Target
(define-fun cns_target_AliceConsent () (TValue Bool)
	(equalString const_Alice n_resource/patient-id)
)
;##### Policy Obligations
(define-fun cns_obl_permit_AliceConsent ()  Bool
	 (and true))
 
(define-fun cns_obl_deny_AliceConsent ()  Bool
	 (and (and
...
;##### Policy Combining Algorithm
(define-fun cns_AliceConsent_cmb_ePreeDis_permit () Bool
	 (or cns_ePre_permit cns_eDis_permit)
)

(define-fun cns_AliceConsent_cmb_ePreeDis_deny () Bool
	 (or 
		 (and cns_ePre_deny cns_eDis_deny)
		 (and cns_ePre_deny cns_eDis_notApp)
		 (and cns_ePre_notApp cns_eDis_deny)
	 )
)

(define-fun cns_AliceConsent_cmb_ePreeDis_notApp () Bool
	 (and cns_ePre_notApp cns_eDis_notApp)
)

(define-fun cns_AliceConsent_cmb_ePreeDis_indet () Bool
	 (or 
		 (and cns_ePre_indet (not cns_eDis_permit))
		 (and (not cns_ePre_permit) cns_eDis_indet)
	 )
)

 
(define-fun cns_AliceConsent_cmb_final_permit () Bool
	 (or cns_AliceConsent_cmb_ePreeDis_permit cns_ruleDeny_permit)
)

(define-fun cns_AliceConsent_cmb_final_deny () Bool
	 (or 
		 (and cns_AliceConsent_cmb_ePreeDis_deny cns_ruleDeny_deny)
		 (and cns_AliceConsent_cmb_ePreeDis_deny cns_ruleDeny_notApp)
		 (and cns_AliceConsent_cmb_ePreeDis_notApp cns_ruleDeny_deny)
	 )
)

(define-fun cns_AliceConsent_cmb_final_notApp () Bool
	 (and cns_AliceConsent_cmb_ePreeDis_notApp cns_ruleDeny_notApp)
)

(define-fun cns_AliceConsent_cmb_final_indet () Bool
	 (or 
		 (and cns_AliceConsent_cmb_ePreeDis_indet (not cns_ruleDeny_permit))
		 (and (not cns_AliceConsent_cmb_ePreeDis_permit) cns_ruleDeny_indet)
	 )
)
...
;PERMIT
(define-fun cns_AliceConsent_permit () Bool
	(and 
		(isTrue cns_target_AliceConsent)
		cns_AliceConsent_cmb_final_permit
		cns_obl_permit_AliceConsent
	)
)
;DENY
(define-fun cns_AliceConsent_deny () Bool
	(and 
		(isTrue cns_target_AliceConsent)
		cns_AliceConsent_cmb_final_deny
		cns_obl_deny_AliceConsent
	)
)
;NOT APP
(define-fun cns_AliceConsent_notApp () Bool
	(or
		(or (isFalse cns_target_AliceConsent) (miss cns_target_AliceConsent))
		(and (isTrue cns_target_AliceConsent) cns_AliceConsent_cmb_final_notApp)
	)
)
;INDET
(define-fun cns_AliceConsent_indet () Bool
	(or 
		...
		(and 
			(isTrue cns_target_AliceConsent)
			cns_AliceConsent_cmb_final_permit
			(not cns_obl_permit_AliceConsent)
		)
		(and 
			(isTrue cns_target_AliceConsent)
			cns_AliceConsent_cmb_final_deny
			(not cns_obl_deny_AliceConsent)
		)
	)
)
;################### END TOP-LEVEL POLICY AliceConsent CONSTRAINTs #########################
