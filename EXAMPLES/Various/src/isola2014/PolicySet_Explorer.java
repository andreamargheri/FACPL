	...
	private class Rule_stopExploration extends Rule {
		...
		Rule_stopExploration() {
			...
			addTarget(new ExpressionBooleanTree(ExprBooleanConnector.AND,
					...
					new ExpressionBooleanTree(new ExpressionFunction(it.unifi.facpl.lib.function.comparison.Equal.class,
							"victimPerceived", new AttributeName("action", "item")))));
			// Obligations
			addObligation(new Obligation("put", Effect.PERMIT, ObligationType.M, "stop"));
			addObligation(new Obligation("put", Effect.PERMIT, ObligationType.M, "role", "rescuer"));
		}
	}
			...
			addTarget(new ExpressionBooleanTree(ExprBooleanConnector.AND,
					new ExpressionBooleanTree(new ExpressionBooleanTree(ExprBooleanConnector.AND,
							new ExpressionBooleanTree(
									new ExpressionFunction(it.unifi.facpl.lib.function.comparison.Equal.class, "qry",
											new AttributeName("action", "action-id"))),
							new ExpressionBooleanTree(
									new ExpressionFunction(it.unifi.facpl.lib.function.comparison.Equal.class,
											"isMoving", new AttributeName("action", "item"))))),
					new ExpressionBooleanTree(
							new ExpressionFunction(it.unifi.facpl.lib.function.comparison.GreaterThan.class, 20,
									new AttributeName("subject", "batteryLevel")))));
			// Obligations
