...
public class FCloud {
	...
	/**
	 * Setting up the simulation environment. Each Hypervisor has 10 units of resource and VM TYPE_1 and TYPE_2 costs 1 and 2 units respectively.
	 * @param absPath Path of the folder where the files for Cloud simulation are places
	 */
	...
	private void createSystemProp(File system_folder) {
		...
		try {
			...
			props.store(new FileOutputStream(system_folder.getAbsoluteFile() + File.separator  + "system.properties"), null);
		...
		for (int i=1; i <= _hype ; i++){
			...
			try {
				prop_hype.store(new FileOutputStream(
						new File (system_folder.getAbsoluteFile() + File.separator  + "HYPER_"+ i +".properties")), null);
			} catch (Exception e) {
				e.printStackTrace();
			} 
		}
