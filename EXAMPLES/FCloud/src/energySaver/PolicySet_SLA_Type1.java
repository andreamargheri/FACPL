...
public class PolicySet_SLA_Type1 extends PolicySet {
	public PolicySet_SLA_Type1() {
		...
		addTarget(
						...
						new ExpressionBooleanTree(
								new ExpressionBooleanTree(ExprBooleanConnector.OR,
										new ExpressionBooleanTree(new ExpressionFunction(
												it.unifi.facpl.lib.function.comparison.Equal.class, "P_1",
				...
				new ExpressionBooleanTree(new ExpressionFunction(it.unifi.facpl.lib.function.comparison.Equal.class,
						"TYPE_1", new AttributeName("resource", "vm-type")))));
		// PolElements
		addPolicyElement(new Rule_hyper_1());
		addPolicyElement(new Rule_hyper_2());
		// Obligation
		addObligation(
				new Obligation("log", Effect.DENY, ObligationType.O, "Not enough available resources for TYPE_1 VMs"));
	}
	...
	private class Rule_hyper_1 extends Rule {
		...
		Rule_hyper_1() {
			...
			// Target
			addTarget(new ExpressionFunction(it.unifi.facpl.lib.function.comparison.LessThanOrEqual.class, 1,
			...
			addObligation(new Obligation("create", Effect.PERMIT, ObligationType.M, "HYPER_1",
					new AttributeName("system", "vm-id"), "TYPE_1"));
		}
	}
