...
public class PolicySet_Release_Policies extends PolicySet {
	public PolicySet_Release_Policies(){
		...
		//Target
		addTarget(new ExpressionBooleanTree(ExprBooleanConnector.AND,new ExpressionBooleanTree(new ExpressionFunction(it.unifi.facpl.lib.function.comparison.Equal.class, "RELEASE",new AttributeName("action","action-id") 
		)
		),new ExpressionBooleanTree(new ExpressionBooleanTree(ExprBooleanConnector.OR,new ExpressionBooleanTree(new ExpressionFunction(it.unifi.facpl.lib.function.comparison.Equal.class, "P_1",new AttributeName("subject","profile-id") 
		)
		...
		private class Rule_hyper_1_release extends Rule {
				...
				Rule_hyper_1_release (){
					...
					//Target
					addTarget(new ExpressionFunction(it.unifi.facpl.lib.function.comparison.In.class, new AttributeName("resource","vm-id") 
