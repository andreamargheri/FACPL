...
public class SMT_LIBGenerator_PermitOver extends SMT_LIBGenerator_Alg {
  @Override
  ...
  public String getCombinationDeny(final String prev, final String current, final String p_name, final Boolean isFirst) {
    ...
    str.append(((((("\t\t (and cns_" + n_cns) + prev) + "_deny cns_") + current) + "_notApp)\n"));
    str.append(((((("\t\t (and cns_" + n_cns) + prev) + "_notApp cns_") + current) + "_deny)\n"));
    str.append("\t )\n");
    return str.toString();
  }
  ...
  public String getCombinationIndet(final String prev, final String current, final String p_name, final Boolean isFirst) {
    ...
    final StringBuffer str = new StringBuffer();
    str.append("\t (or \n");
    str.append(((((("\t\t (and cns_" + n_cns) + prev) + "_indet (not cns_") + current) + "_permit))\n"));
    str.append(((((("\t\t (and (not cns_" + n_cns) + prev) + "_permit) cns_") + current) + "_indet)\n"));
    str.append("\t )\n");
    return str.toString();
  }
