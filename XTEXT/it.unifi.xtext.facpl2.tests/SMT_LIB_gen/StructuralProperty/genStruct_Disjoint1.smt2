...
(declare-const n_action/type (TValue String))
(assert (not (and (miss n_action/type) (err n_action/type))))
...
(define-fun cns_target_r1 () (TValue Bool)
	(equalString const_read n_action/type)
)
;##### Rule Obligations
...
(define-fun cns_target_r12 () (TValue Bool)
	(FOr (equalString const_read n_action/id) (equalString n_subject/id const_Andrea))
)
;##### Rule Obligations
...
;###################### STRUCTURAL PROPERTY #####################
(echo "--> Check NameDIJOINTName1... (holds if the following four checks are unsat)")
(echo "...(1/4) Name_PERMIT  DIJOINT Name1_PERMIT...(holds if it is unsat)")
 (push)
(assert 
	 (and cns_Name_permit cns_Name1_permit)
)

(check-sat)
;(get-model)

(echo "...(2/4) Name_PERMIT  DIJOINT Name1_DENY...(holds if it is unsat)")
 (push)
(assert 
	 (and cns_Name_permit cns_Name1_deny)
)

(check-sat)
;(get-model)

(echo "...(3/4) Name_DENY  DIJOINT Name1_PERMIT...(holds if it is unsat)")
 (push)
(assert 
	 (and cns_Name_deny cns_Name1_permit)
)

(check-sat)
;(get-model)

(echo "...(4/4) Name_DENY  DIJOINT Name1_DENY...(holds if it is unsat)")
 (push)
(assert 
	 (and cns_Name_deny cns_Name1_deny)
)
