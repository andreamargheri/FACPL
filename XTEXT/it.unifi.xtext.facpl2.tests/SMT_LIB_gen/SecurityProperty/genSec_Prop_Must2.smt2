...
;################### STRING DECLARATIONs #######################
 (declare-datatypes () ((String s_Franco s_read s_Andrea  s_AdditionalStringValue )))
...
(declare-const const_Franco (TValue String))
(assert (= (val const_Franco) s_Franco))
(assert (not (miss const_Franco))) 
(assert (not (err const_Franco)))
...
(define-fun cns_target_r1 () (TValue Bool)
	(FOr (equalString n_action/id const_read) (equalString n_subject/id const_Andrea))
)
;##### Rule Obligations
...
(assert (= (val n_subject/id) s_Franco))
(assert (not (miss n_subject/id)))
(assert (not (err n_subject/id)))

(assert (not cns_Name_permit))(echo " --> Check MUST property... (holds if the following check is unsat)")
