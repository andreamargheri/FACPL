...
public class FulfilledObligationCheck extends AbstractFulfilledObligationCheck implements Cloneable {
	 ...
	 */
	public FulfilledObligationCheck(Effect evaluatedOn, ExpressionFunction target, ExpressionFunction status_target,
			int expiration) {
		super(evaluatedOn);
	...
	public FulfilledObligationCheck(Effect evaluatedOn, ExpressionBooleanTree target, ExpressionFunction status_target,
			int expiration) {
		super(evaluatedOn);
		this.target = target;
		this.target = new ExpressionBooleanTree(status_target);
	...
	public FulfilledObligationCheck(Effect evaluatedOn, ExpressionFunction target, ExpressionBooleanTree status_target,
			int expiration) {
		super(evaluatedOn);
		this.target = new ExpressionBooleanTree(target);
		this.status_target = status_target;
	...
	public StandardDecision getObligationResult(ContextRequest cxtRequest) {
		...
		/*
		 * This method evaluate target and status target and according to result
		 * return: IF target and status target TRUE -> EVALUATED_ON [PERMIT OR
		 * DENY] IF target or status target FALSE -> NOT APPLICABLE IF target or
		 * status target BOTTOM -> NOT APPLICABLE IF target or status target
		 * ERROR -> INDETERMINATE
		 */
	...
	public Object clone() throws CloneNotSupportedException {
		 ...
		 */
		return new FulfilledObligationCheck(this.evaluatedOn, this.target, this.status_target, this.originalExpiration);
	}
